...
#Reservation c'est un dictionnaire 
def notInReservation(res,noeud):
    (x,y)=noeud.etat
    t=noeud.t
    if (x,y) in res.keys():
        if t in res[(x,y)]:
...
class Noeud:
	def __init__(self, etat, g, t,pere=None):
		self.etat = etat
		self.g = g
		self.pere = pere
		self.t=t
	...
	def expand(self,p):
		...
		"""
		nouveaux_fils = [Noeud(s,self.g+p.cost(self.etat,s),self.t+1,self) for s in p.successeurs(self.etat)]
		return nouveaux_fils

	def expandCooperative(self,p,grilleEnnemis,etatInitiale):
		""" étend un noeud avec ces fils
		pour un probleme de taquin p donné
		"""
		nouveaux_fils = [Noeud(s,self.g+p.cost(self.etat,s),self.t+1,self) for s in p.successeursCooperative(self.etat,grilleEnnemis,etatInitiale)]
		return nouveaux_fils
...
def astarcooperating(p,reservation,grilleEnnemis,temps,verbose=False,stepwise=False):
    """
    ...
    nodeInit = Noeud(p.init,0,temps,None)
    ...
    # Hypothèse de consistence de l'heuristique        
        ...
        if p.immatriculation(bestNoeud.etat) not in reserve:            
            ...
            nouveauxNoeuds = bestNoeud.expandCooperative(p,grilleEnnemis,nodeInit.etat)
            #print("FROTIIERREE **********",len(nouveauxNoeuds))
            for n in nouveauxNoeuds:
                if notInReservation(reservation,n): # si le neoud n'est pas pris par l'un des agent de mon équipe 
                    f = n.g+p.h_value(n.etat,p.but)
                   # print("######################",n.etat)
                    heapq.heappush(frontiere, (f,n))
                    #print("FROTIIERREE **********",len(frontiere))
