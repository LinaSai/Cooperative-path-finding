...
class Noeud:
    ...
    def expand_slicing(self,p,grille):
        nouveaux_fils = [Noeud(s,self.g+p.cost(self.etat,s),self) for s in p.successeurs_slicing(self.etat,grille) ]
        return nouveaux_fils
...
def astar_path_slicing(p,grille,verbose=False,stepwise=False):
    """
    application de l'algorithme a-star
    sur un probleme donné
        """
    ...
    while frontiere != [] and not p.estBut(bestNoeud.etat): #en cas de collision, changer le but de p pour une case du path calculé au debut.            
        (min_f,bestNoeud) = heapq.heappop(frontiere)
    ...
    # Hypothèse de consistence de l'heuristique        
        ...
        if p.immatriculation(bestNoeud.etat) not in reserve:            
            reserve[p.immatriculation(bestNoeud.etat)] = bestNoeud.g #maj de reserve
            nouveauxNoeuds = bestNoeud.expand_slicing(p,grille)
            for n in nouveauxNoeuds:
                f = n.g+p.h_value(n.etat,p.but)
                heapq.heappush(frontiere, (f,n))
